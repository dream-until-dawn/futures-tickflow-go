...
func TestNightArtifactFollowsCoversFrom(t *testing.T) {
	...
	//
	// ⚠️ **这一条的理由改过一次，而旧理由是【假】的**（评审方 2026-09-09 实测，我复核一致）：
	// 旧理由写「上面两个子测试各自都可能通过而这一条仍然不成立（比如实现改成『排除头两天』时）」，
	// 而实测那个突变 ⇒ **红 3 条，本条不在其中** —— 因为**本函数一次都没调 ScanNightAbsent**。
	// ⇒ 一个**不可能发生**的失败，不构成一条断言的理由（本仓那条判据）。
	//
	// ✅ 而它仍然值得留，理由换成这一句：
	// **把那个【差】在代码里写成一句话，让读的人看见断言的是【差】，不是两个值。**
	// 断言两个端点 ≠ 断言它们的差 —— 而「跟着走」说的正是那个差。
