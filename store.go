...
var (
	...
	// 它说的是**本次没走查过**，**不表示这一段有问题** —— 详见 `gap.go` 的 `GapStoreVerifyUnrun`。
	//
	// ⛔ **上一版这里写的判别符是「走一遍，看缺口动没动」，而它是【循环】的**（实测）：
	// 今天第二次同步会在落盘那一步就中止，**根本走不到走查** ⇒ 缺口逐字不变。
	// **「走一遍」得到的是同一句话，不是新信息。**
	//
	// —— ⚠️ 它与 `GapStoreVerifyUnrun` 【名字对不齐】，而那是有意的（(p) 只改了类别）——
	//
	// 两个名字讲的是**两件事**，分工写在这儿，免得下一个人以为「改了一个忘了另一个」：
	//
	//	这个哨兵（store 层）  **状态**：这一段【没有被走查过】——
	//	                      问它的是 `DaysWithBars`，而它答的是「我现在答不了」
	//	那个类别（Sync 层）   **本次事件**：这一遍走查【没跑成】——
	//	                      (iv) 之后它只剩一个产生者：`VerifyCoverage` 跑不起来
	//
	// ⇒ 映射仍在（`gap.go` 里 `errors.Is(s.Err, ErrSpanUnverified)` ⇒ `GapStoreVerifyUnrun`），
	// 而那不是「同一件事的两个名字」：**一个是库的状态，一个是这一次的事件** ——
	// 走查没跑成 ⇒ 那些段当然仍处在「没被走查过」这个状态里，所以映射成立。
	//
	// 📎 判据用本仓那条：**判两个东西该不该共用一个名字，看【处置】分不分岔** ——
	// 这两者**处置相同**（都去看 `TruncatedTails` 里那条「这一遍走查没能跑起来」），
	// 所以它们可以映射；**而它们的【主语】不同**，所以不该合并成一个名字。
...
// 源不可重放时，自愈从来就不在选项里。
//
// 不可重放那一支返回 ErrLegacyMeta：它要的是**一个显式决定**
// （人工确认后删掉该周期的 .dat/.meta 再重拉），而不是一个自动动作。
// ⚠️ 这句原来写「Force 或人工确认」—— `SyncRequest.Force` 已于 2026-09-12 删除
// （它没有任何代码读它），见 syncer.go 那段说明。
//
