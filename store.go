...
// 本文件对应 docs/design.md §一 的 `store.go`：Store 接口所在（设计里写过的 Iterator 没有落地）。
//
...
// **而 Store 接口本身仍未定型**（2026-09-14 注：这是 09-09 的进度；今天定型的是 8 个方法，还缺读口子，v0.6 定） —— 这不是漏了，是写下来的决定，
...
type Store interface {
	...
	// ⛔ **这里没有 Walk**（用户 2026-09-15 裁定）：逐根读的口子只在 `*segfile.Store` 上（store/segfile/walk.go），
	// 不进本接口 —— 它在 v0.6 里没有生产消费者，放进接口会让第三方实现在 v0.6.0 编译不过。
	// v0.7 的 calendar/derived 落地、有了消费者验证形状之后，再定要不要进接口（design.md §十五「第四次裁决」）。
