...
const (
	...
	// —— ✅ 今天只剩两个产生者，而它们的处置【不同】——
	//
	//	一、**这一遍走查跑不起来**（`VerifyCoverage` 的第二个返回值非 nil，例如读盘失败）
	//	    ⇒ 那时 `TruncatedTails` 里有一条「这一遍走查没能跑起来：…」
	//	    ⇒ 处置：**看那条真因**（盘/权限/文件被占）。**重跑是对的** —— 它可能是瞬时的。
	//	二、**这个 `Store` 实现没有为这一段给出结论**（违约：契约要求它是「全的」）
	//	    ⇒ 那时 `TruncatedTails` 里**什么都没有**
	//	    ⇒ 处置：**去看那个 Store 的实现**。⛔ **重跑没有用** —— 它下次还是不会给。
	//
	// 🔴 ⇒ **判别这两者只要看一个地方：`TruncatedTails` 里有没有那条「跑不起来」。**
	// ⚠️ 而「走一遍就行」这句话**已经从这里删掉了**：它对情形二是**循环**的
	// （判别符与被判别的状态是同一个东西：再走一遍，那个 Store 还是不给结论）。
