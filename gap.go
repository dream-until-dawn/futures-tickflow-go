...
const (
	...
	//
	// ⚠️ **而「不表示这一段有问题」这句话有两种【为真而有害】的情形，今天都存在**：
	//
	...
	//
	// —— 现状如此。(iv) 会把走查绑到【库】上、并把全库错误归给每一段，那时这两句要重写。
	//
	// ⛔ **曾经有第三种，2026-09-11 已经修掉了**：一次**完全成功**的多块同步也会产出这一类
	// （`touched` 里是【分块】的 span，而 `Coverage()` 给的是并段之后的值，两者做 map 键时不相等）。
	// ⇒ `verifyTouched` 现在走查「与 `touched` 相交的 `Coverage()` 段」，键与读取侧同源。
	// 🔴 写在这儿是因为：**在 `cffexsource` 的出厂值（`BatchDays: 1`）下，那时候
	// 一次成功的同步就会产出这一类** ⇒ 上面那句「走一遍就行」当时**不是偶尔有害，是必然有害**
	// （照做 ⇒ 第二次 `Sync` 重拉 ⇒ 落盘失败）。**它是在那一次修复之后才重新为真的**，
	// 别把它读成「一直都对」。（`false_unverified_test.go` 钉着修完之后的样子。）
	//
