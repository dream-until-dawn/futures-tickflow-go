...
//
// ⛔ **不排序，报错。** 而理由分成两句，**它们的证据强度不同**：
//
//	策略    排一次就把「上游变了」这个事实抹掉了，而它恰恰是最该被看见的一类变化。
//	        ⚠️ **这一句不需要证据** —— 它是一个取舍，不是一个经验断言。
//	误报率  实测 2026-09-09，三份 fixture 共 499 行：**一直升序、无重复日期**。
//	        ⇒ 它支持的只是**更弱也够用**的一句：**这条今天不会天天响。**
//
// ⚠️ 两句原本连着写，读起来像「因为量过 499 行，所以可以不排序」——
// **而真正的理由和样本量无关。**（这一拆是评审方 2026-09-09 提的。）
...
//	来源标记    FlagSrcSina
//
// ⛔ **「这一天没有 Bar」有三种原因，而本函数只对前两种出声。**
//
// 我自己曾把它归成两路（「覆盖不到」／「覆盖得到而那天没交易」）——
// **那个分类读起来穷尽，实际漏了第三格**（评审方 2026-09-09 指出）：
//
//	甲  区间有一端落在日历覆盖之外                  ⇒ ErrCalendarGap              ✅ 炸
//	乙  区间内、**有这一行**、而日历说那天非交易日     ⇒ ErrSinaDisagreesWithCalendar ✅ 炸
//	丙  区间内、日历说**是**交易日、而新浪**没有这一行** ⇒ **静默**                   ⛔
//
// ⚠️ 丙格**根本走不到 DayOf**：循环是 `for i, r := range rows` ——
// **按行走，不按日走。** 没有那一行，那一天从不被访问，**没有任何一条路会经过它。**
// ⇒ 所以它不是「判据写松了」，是**那一格在结构上没有被遍历到**（登记⑨）。
//
// ⚠️ 而这个漏掉本身和它描述的东西同形：
// **一个读起来穷尽的分类，漏掉的正好是「该有、却没有」的那一格。**
//
