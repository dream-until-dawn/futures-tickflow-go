...
func stripJSONP(b []byte) ([]byte, error) {
	...
	//
	// ⚠️ **射程**（评审方 2026-09-09 构造 8 类畸形输入实测，8 类全部【报错】，无一静默）：
	//
	//	残余的那一类是可描述的，不是「大概没有」：**静默切错要求
	//	「前缀里第一个 `(` 之后那一段，自己就是一个合法的 []rawRow」** —— 构造不出来。
	//
	//	⛔ 而这里只掐【开头那一段】块注释 —— **那是「射程写成位置」的形状**。
	//	  注释不在开头、有两段注释、`//` 行注释带括号 …… 今天都不出事，
	//	  **而理由是那些偏移都撞成了 JSON 语法错，不是因为射程够宽。**
