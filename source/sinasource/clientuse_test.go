...
//
// 🔴 **它存在的理由是一处【承诺搬家】**（与 cffexsource 那条同形，理由一字不差）：
...
//
// ⚠️ 射程照写：**它只证得了「本源今天说的是实话」。**
// 一个刻意说谎的第三方源不会来写这条测试 —— 那一格由 `ClientUse` 的注释声明为射程。
//
// ⚠️ 而本源与 cffexsource **一次 `Bars` 的请求数不同**（这边 1 次、那边 N 次），
// 所以断言写成「注入的 client 用了几次 == 对端收到几次」——
// **不写死一个数**：写死的话，`BatchDays` 那一格的差别会让两边的这条测试长得不一样。
func TestDeclaredClientUseMatchesBehaviour(t *testing.T) {
	fs := newFixtureServer(t)

	// 前提一：本源**声明**的是 ClientUseHTTP。
	// ⛔ 不立这一条，下面那个「走了几次」就不知道该拿去和什么对。
	base := newTestClient(t, fs)
	if got := base.Caps(rbReq().Symbol.ProductKey()).ClientUse; got != tickflow.ClientUseHTTP {
		t.Fatalf("本源声明的是 %v，而这条测试是按 ClientUseHTTP 写的 —— 前提变了", got)
	}
	fs.hits = 0 // 上面那次构造不发请求；清零是为了让下面的计数只含本次

	rt := &countingRT{next: http.DefaultTransport}
	c := newTestClientWithHTTP(t, fs, &http.Client{Transport: rt})

	if _, err := c.Bars(context.Background(), rbReq()); err != nil {
		t.Fatalf("Bars：%v", err)
	}
	...
	if rt.n != fs.hits {
		t.Errorf("注入的 client 被用了 %d 次，而对端收到 %d 次请求\n"+
			...
			"    编排那一层【查不出这件事】—— 所以它只能在这儿被查", rt.n, fs.hits)
	}
}
