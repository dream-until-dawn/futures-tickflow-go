...
// ⇒ 修好它需要【上市日】。
//
// ⛔ 【2026-09-10 更正】此处原来把出处指向那个参考数据上游，并写了一句
// 「那一天到了，这条测试会红」—— **而那一天经这条路永远不会到**：
// 全量实测那个上游的 47 个字段里没有上市日（docs/probe.md「复核 v5」）。
//
// 🔴 于是这条测试原来挂着一个**永远不会触发的到期条件** ——
// 本仓那条说的是「已到期与还活着，外形一模一样，只能当场求值」，
// **而这一条更糟：它永远不到期，所以连「求值」都不会有人去做。**
// ⚠️ 而它还把后续动作一起搁浅了（「红了之后要把它改成按品种各不相同，并关掉㉔」）。
//
// ⇒ 到期条件改成**当场可求值**的一句：
//
//	**在本仓出现第一个能按品种给出上市日的出处之前，这条测试就该是绿的。**
//	求值法：数一数那个替代出处落地了没有 —— 今天 0。
//	（替代出处已实测：中金所 XML 逐日列合约，二分找边界；
//	  ⚠️ 而它的单调性前提未验，所以只写在探针文档里、不进任何接口。）
//
// ⇒ 那一天到了之后要做的仍是：把它改成「按品种各不相同」，并关掉㉔。
