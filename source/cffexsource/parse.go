...
// 🔴 **它是「那天还没出数」这一格的唯一守卫**（2026-09-16 实测，probe.md 七之一）：
// 未收盘的当天、周六、未来日期，中金所都返回 HTTP 200 ＋ 同一张 2132 字节 HTML
// ⇒ `fetchDay` 的非 200 检查一次都没生效，是这里的 XML 校验把它变成【报错】而不是【0 根】。
// ⇒ 少了它，那一天会被登记成「拉过确认没有」（v0.6.0 发布说明勘误三那个形状）。
