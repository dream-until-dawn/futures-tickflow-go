...
for f in FILES:
    ...
    #     ⇒ 仪器是好的，这一格没有守卫。）
    #
    #   ⚠️ 这个洞有多大 ——【2026-09-08 在 a4d24cf 上实测】：
    #     五个载体里【#### 及更深】共 25 处（#### 22 ／ ##### 2 ／ ###### 1）；
    #     登记表 263 条规矩中，有 96 条的【最近标题是 #### 或更深】—— 也就是
    #     它们的 section 字段只记到外层的 ##/###，位置里有一段没被记下来。
    ...
    #     量法（照着能重算）：分母 = 本文件的 is_rule 加下面那条 12 字过滤，
    #       也就是 quotedRules 的条数；分子 = 其中「从该行往上找到的第一个
    #       任意级标题」深度 >= 4 的条数。
