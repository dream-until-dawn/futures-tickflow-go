...
#
# ⚠️ 实测（评审方提出，我独立复现）：**全部 ##/### 小节里，锚点【借自更深子节】
#   的 = 0 个**（2026-09-08 / f0eea38 复测，仍为 0）。
#
#   ⚠️ 节数【故意不写在这儿】：它每次重造都会打印，而抄在注释里的那份注定掉队——
#     上一版这里写的是「54 个小节」，两个提交之后（b360b0c 加了一个小节）它就是 55 了，
#     **是被后面的提交弄假的，不是当初写错了**。是评审方核尖端时发现的。
#     ⇒ **会漂的数用指代，不漂的结论才写成数。**「借用 = 0」是结论，节数是随手的分母。
#   **但那是运气，不是设计**——写一个 ### 小节、让它的第一段内容就是一个 ####
#   子节，它的锚点就会被静默地借过去，而本脚本读的这五个载体里【#### 及更深】
#   已有 25 处（#### 22 ／ ##### 2 ／ ###### 1；2026-09-08 / f0eea38 实测，
