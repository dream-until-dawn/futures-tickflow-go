...
def main():
    ...
    WANT_HEAD = "### 6.1 "
    WANT_TAIL = "## 七、同步"
    # ⛔ 这两条只对【工作区】断言，历史点一律放行（评审方 2026-09-09 报的，
    # 而这条账是他要来的那个修法带出来的，他先说了出来）：
    #
    #	这两个常量记的是【今天的章节结构】。而历史点的章节结构是另一回事，
    #	并且它**不可能再变** —— 对一个不可变的东西断言一个可变的常量，
    #	只会在常量变的那天把它们一起打死。
    #
    # 实测：把 WANT_TAIL 改成新章节名（即一次合法重构 + 按注释同步改常量）
    # ⇒ 1f62620 / c79cb59 等历史点**全部读不回来**。
    # 而 `<SHA>` 那条路不是可有可无的：**它是「在四个点上各数一遍」的唯一实现方式，
    # 也就是当初论证换范围的全部证据来源。**
    #
    #	⇒ 断言的射程是「工作区有没有漂」；历史点要的是「照当时的样子算回来」。
    #	  **两件事，一道门 —— 所以这道门只对工作区关。**
    assert ref is not None or c["head"].startswith(WANT_HEAD), (
        "§6.1 的标题变了：%r（期望以 %r 开头）—— 改了就把这里的常量一起改"
        % (c["head"][:60], WANT_HEAD))
    assert ref is not None or c["tail"].strip() == WANT_TAIL, (
    ...
    if ref is None:
        ...
        print("  （历史点：不对两端断言 —— 那个结构已经不可能再变，见代码注释）")
