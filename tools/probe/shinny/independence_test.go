...
var mustNotImportLibrary = []string{
	...
	"livetail.go",    // v0.10 起手 6.36：推送的收盘后改动与冻结 —— 量外部协议，不许拿本库判定（v0.10 的判完结要由它的读数来定）
	"liveskew.go",    // v0.10 P-b0 6.38：本机偏差告警的统计量 —— 回放落盘的推送帧，读数要定 Live 的取法，不许拿本库判定
	"liveguard.go",   // v0.10 L6 重议（6.38 六）：守 G 的前提的离线验证 —— 回放落盘的推送帧，不许拿本库判定
}
