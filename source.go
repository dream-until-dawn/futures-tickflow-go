...
//
// ⛔ **而它的【代价】必须和它一起读 —— 上一版只写了好处，那正是本仓刚记过的形状**
// （「残余射程说的都是真的，而漏了后半句」）：
//
//	声明 `ClientUseNone` ⇒ **闸门那条检查对这个源整个关掉**
//	⇒ 一个声明 None 却照样用【自己的】client 发 HTTP 的源，绕过闸门而**报告一个字不说**
//	⇒ `3dc406e` 抓的那个假绿，**对它恢复了**
//
// 实测四格（2026-09-10）：
//
//	声明 HTTP + 走我们的   闸门 5 次 · Ungated 0 条 · Complete=true   ✅
//	声明 None + 用自己的   闸门 0 次 · Ungated 0 条 · **Complete=true** ⛔ 绕过且静默
//	声明 None + 走我们的   闸门 5 次 · Ungated 0 条 · Complete=true   （无害，但见下）
//	声明 HTTP + 用自己的   闸门 0 次 · Ungated 1 条 · Complete=false  ✅
//
// ⇒ **换句话说：我们把一个【会误报的检查】换成了一个【可以被一句声明关掉的检查】。**
// 这个交换仍然值得做，理由三条：
//
//	一 方向  之前是必然误报（而长期误报会关掉它自己），之后是「要写一行谎才失效」
//	二 形状  同 `NoPacing()` / `BatchDaysUnbounded` / `NoTimeout` ——
//	  **不消灭那个选择，逼它变成一个写得出来、看得见的动作**
//	三 位置  ⇒ **「源有没有说实话」在【源自己那一层】是可核的**：
//	  一个源的自测可以断言「我声明了 HTTP，那我的 Bars 确实走了注入的那个 client」。
//	  ⇒ 又是那条：**「够不到」是一句带地址的话** —— Syncer 够不到，而源够得到。
//
// ⚠️ 而**有害的那个方向不可证**（声明 None ＋ 真绕过：从 Syncer 这一层看，
// 它与「不适用」同形）。**能查的就查，查不了的写下来** —— 这一段就是那个「写下来」。
type ClientUse int
...
type Capabilities struct {
	...
	// MaxBars 是单次最多给多少根。
	//
	// ⛔ **【2026-09-10 补前提】这里原来只写「新浪 1023，且【无法翻页】」——
	// 而那 1023 是【分钟线】的硬顶，而 sinasource 这一版【不给分钟线】。**
	// ⇒ 也就是说：**这句解释拿的是本源今天没有的那个能力**，那个数今天不生效。
	// 🔴 本仓那条：**一句理由会被下一个人当检查表用** ——
	// 下一个给本源加分钟线的人，会把「1023 已经写在这儿了」读成「这一格已经处理过」。
	// ⚠️ 而它今天够不到任何行为（见下面那条登记：非测试读点 0）⇒ 补的是前提，不是数。
	MaxBars int
...
// —— 登记：四个【今天没有消费者】的字段 ——
//
// 2026-09-10 双方各自量了一遍（评审方用字段选择器重取，不复述我的数）：
//
//	字段        非测试且非本文件的读点   测试里的读点
//	MaxBars     **0**                  1
//	HasSettle   **0**                  6
//	Realtime    **0**                  1
//	HasOI       **0**                  **0**
//
// ⭐ `HasOI` 连测试里都是 0。两条互相独立的路都指向它：
//
//	`\.HasOI\b`        全仓 **0**
//	`\bHasOI\b`（裸标识符）全仓 **3** ⇒ 逐条看：**1 处声明 ＋ 2 处赋值 ＋ 0 处读取**
//
// 🔴 ⇒ **一个源在这一格上说谎，今天全仓没有任何东西能证伪它。**
// 这正是本仓 ⑫ 那一族：**可证的谎、零误报、而今天没人报。**
//
// ⛔ **而现在【不】给它们加守卫** —— 那是往没有收益的地方加摩擦，
// 而本仓记过：**一个长期不产生价值的检查最终会被关掉，而它被关掉时会顺手带走别的。**
//
// ⇒ 登记成一条**当场可求值**的到期条件（形状与 refdata 那条同型）：
//
//	到期条件：**出现第一个【读】这些字段的消费者之前**
//	求值法：  `grep -rn "\.MaxBars\b\|\.HasSettle\b\|\.HasOI\b\|\.Realtime\b" --include=*.go .`
//	         数**非测试文件**里的命中 —— 今天 **0**
//	那一天：  这些字段第一次变成**契约** ⇒ 才需要「源不许在这一格上说谎」的守卫
//
// 🔴 形状：**零消费者时它是一个设计选择；第一个消费者出现那天，它变成一句承诺。**
// ⚠️ 而这条到期条件今天**没有做成会红的测试**（不像 ㉒ 与 refdata 那两条）——
// 写下这一点，是因为「有没有被做成测试」这件事本身就是那三条之间的**不齐**，
// 而不齐要一眼看得见。
