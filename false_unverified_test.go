...
//
// —— 📌 而这一片里两种写法并存，判据是【这次修复推不推迟】，不是「哪种更好」——
//
//	钉住缺陷（断言现状 ＋ 红法二写「有人修好了」）
//	  ⇐ 只在**修复被推迟**时才是对的写法。它存在的唯一理由是：**红的测试进不了 main**。
//	  ⇒ `whole_library_error_unattributed_test.go` 就是这一种（它的修复在 (iv)）。
//	直接断言修好之后的行为
//	  ⇐ **修复就在同一片里**时用这一种：它在【没有那次修复】时会红
//	  ⇒ 那才是「这次改动」的证据。本文件这四格就是这一种。
//
// 🔴 差别在**证明力**：钉住缺陷的那一版在修复面前只会「从绿变红」，
// 它证明的是**有东西变了**，不是**变对了**。
//
// 病在**键**，不在集合：
//
