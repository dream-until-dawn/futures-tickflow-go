...
func closes(vs ...float64) []tickflow.Bar {
	cs := make([]tickflow.Bar, len(vs))
	for i, v := range vs {
		cs[i] = tickflow.Bar{TradingDay: tickflow.TradingDay(i + 1), Open: v, High: v, Low: v, Close: v}
	}
	return cs
}

func hlc(rows ...[3]float64) []tickflow.Bar {
	cs := make([]tickflow.Bar, len(rows))
	for i, r := range rows {
		cs[i] = tickflow.Bar{TradingDay: tickflow.TradingDay(i + 1), High: r[0], Low: r[1], Close: r[2], Open: r[2]}
	}
	return cs
}

// col 取某一路输出的整列。
func col(t *testing.T, ind Indicator, cs []tickflow.Bar, k int) []float64 {
	t.Helper()
	rows := Compute(ind, cs)
...
func TestConventionSelection(t *testing.T) {
	// 默认是 CN —— 用户 2026-09-17 定（照搬姊妹仓配置），**不是**拿国内期货软件量出来的。
...
func randomWalk(n int, seed uint64) []tickflow.Bar {
	cs := make([]tickflow.Bar, n)
	px := 100.0
	s := seed
	...
	for i := range cs {
		...
		cs[i] = tickflow.Bar{
			TradingDay: tickflow.TradingDay(i + 1), Open: o, High: hi, Low: lo, Close: px,
