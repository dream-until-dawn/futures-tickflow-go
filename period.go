...
//
//	日内  永远匹配 ^[0-9]+m$          1m · 5m · 15m
//	日历  没有一个匹配那个形状        1d · 1w · 1mo
//	⇒ 两族的取值空间不相交
//
// ⛔ 这段理由 2026-09-10 被评审方改过一次，而改的那半句【当时是假的】：
// 原文写「日内名里 'm' 之后没有字符，**而三个日历名的第二个字符都不是 'm'**」——
// 后半句为假："1mo" 的第二个字符正是 'm'。结论（不相交）仍然真，靠的是前半句
// （'m' 必须是最后一个字符 ⇒ "1mo" 后面还有 'o' ⇒ 不是日内形状）。
// 🔴 而后半句为假【要紧】：下一个加成员的人会照着那句话检查，
// 而 "3m" 的第二个字符也是 'm' —— 他会以为自己撞了，或反过来放行别的。
// ⇒ 判据：**一句写下来的理由，会被下一个人当成检查表用** —— 结论对不救它。
//
