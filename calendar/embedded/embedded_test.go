...
//
// 这条测试断言的是错误行为，目的是让它【别悄悄变】：修掉它的人必须同时更新 DayOf 的文档与 contract.md 的限定。
//
// ⛔ **2026-09-17 改写（钉子不拔）**：原来这里写「calendar/derived 从分钟数据反推真值修掉它时，这条会变红」——
// 而 v0.7 定了 **derived 只读不写、产物不回灌**（design.md §十五「开工条件二的回答」四格之三）：
// derived 只报「base 说有夜盘而观测没有」这条差异，**不改本包**。⇒ 这条钉子**不会**因为 derived 落地而变红，
// 它钉的仍是一个真缺陷（probe.md 6.24：253 天真实交易日表上复现过；6.30：本库主连在那 6 天里判 c）。
// 修它的路径换了：交易所成文的年度休市安排（probe.md 6.23 之二 / 之三）作为**另一个入口**注入本包，要人拍板 —— 不在 v0.7。
