...
type SyncRequest struct {
	...
	//
	// ⛔ **今天没有任何代码读这个字段**：非测试的**代码行**里读取点 **0** 处。
	// 同一把尺子量 `req.MaxConsecutiveFails` 得 **2**、`req.From` 得 **17** ⇒ 它会报东西。
	// ⇒ **设成 true 什么都不会发生。**
	//
	// 选择器（**最后那一段排掉注释行是必须的**）：
	//
	//	grep -rn "req\.<字段>" --include=*.go . | grep -v _test | grep -v ':[[:space:]]*//'
	//
	// ⛔ 上一版少了那一段，于是撞上两层（评审方 2026-09-11 量，我复现，逐字相同）：
	// 一、**这段说明自己被数了进去** —— 它逐字写着那三个符号 ⇒ 写下它们就把它们改掉了
	//    （含注释：`Force` 0→1 · `MaxConsecutiveFails` 2→3 · `From` 18→19）。
	// 二、**连底座上的标定值也是错的**：`req.From` 真值是 17，那个 18 含了一行注释。
	// 🔴 而换成这把尺子之后，读数在改之前与改之后**相同** —— 因为它量的是**代码**，
	// 而那段说明是**文本** ⇒ **`Force = 0` 不再需要底座**，它在写下这句话的这一刻仍然为 0。
	//
