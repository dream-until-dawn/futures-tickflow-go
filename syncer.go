...
type SyncRequest struct {
	...
	//
	// ⛔ **今天没有任何代码读这个字段**（2026-09-11 实测：非测试代码里 `req.Force` 的
	// 读取点 **0** 处；同一把尺子量 `req.MaxConsecutiveFails` 得 2、`req.From` 得 18
	// ⇒ 尺子会报东西）。⇒ **设成 true 什么都不会发生。**
	//
	// 🔴 它为什么躲过了 doccheck：那道检查守的是「**声明存在**」，而这个声明确实存在。
	// ⇒ 本仓那条「**外形不携带状态**」的新成员，而这次外形是**一个字段的存在性** ——
	// 比以往几格都难看见，**因为声明就是文档要求的那个东西本身**。
	//
	// ⚠️ 而它不起作用的根在更前面：`Sync` 在 fetch 之前**从不查 coverage**
	// （`days` 是请求区间的全部交易日）⇒ 没有「跳过」这件事，`Force` 也就没有东西可忽略。
	// 而 `docs/design.md` 写的是另一件事：「读 Coverage → 求缺失 → **只拉这些**」。
	// ⇒ 这是一次**文档与实现漂开**，登记为待办；在它补上之前，这个字段是一句假承诺。
	//
