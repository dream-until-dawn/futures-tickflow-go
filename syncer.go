...
type SyncRequest struct {
	...
	//
	// ⛔ **显式给的 `To` 不许含一个在 `now` 时还没收盘的交易日**，否则 `Sync` 报错、什么都不做（2026-09-14 起）。
	// 理由：源只给已完结的根，那一天会被登记成「就这么多」，之后的同步把它当成已覆盖、永远不再拉 ——
	// v0.5.0 及之前正是这样（日线把当天记成「拉过、确认没有」，1m 把半天记成一整天），见 v0.6.0 发布说明勘误二。
	// ⇒ 盘中要「同步到最新」，传 `To = 0`。
	To TradingDay

	// MaxConsecutiveFails 是失败预算：**连续**失败几次就停（丙二）。
	//
	// ⛔ 一块失败而预算还够 ⇒ **重试同一块**，不跳过（2026-09-15 起）。v0.5.0 及之前是跳到下一块，
	// 后面的块照登 ⇒ coverage 留洞 ⇒ 此后每次同步停在洞那一块，新交易日永远拉不到（v0.6.0 发布说明勘误四）。
	// 预算用完 ⇒ HaltBudget，失败那一块及其后所有块都不登记。
	//
...
type SyncReport struct {
	...
	// HeldBack 记「源一根没给、而这次没有登记的那几天」（勘误三，2026-09-15）。
	//
	// ⛔ 它**必须存在**，理由与 `SkippedCovered` 同：那几天在 `Gaps` 里是「没拉过」，
	// 而这次同步**明明拉过它们** —— 没有这一栏，「没拉」与「拉了、源没给、在等」共用一句话。
	// ⚠️ 它**不进 `Incidents()`**：源晚一点出数不需要人看，下次同步自己补；处置与「没拉过」相同（再同步一次）。
	HeldBack []string
