...
type Bar struct {
	...
	//
	// ✅ **2026-09-09：这句「会变」拿到了它防的那件事的第一个实例**（`probe.md` 6.10）——
	// `SHFE.rb` 的夜盘在 **2016-05-03** 由「开到 01:00」改成「开到 23:00」，
	// DCE 在 2019-03-29、CZCE 在 2019-12-11 各改一次。
	// **三个日子全都落在本库的目标深度里**（天勤 1m 回溯到 2016-01-04）。
	//
	//	⇒ 这个取舍当初是凭「会变」做的；今天它有了日期，而日期落在【会被读到的数据】里。
	//	  **验完之后维持原判，和没验过就维持原判，不是一回事。**
	...
	// 中金所品种（IF/IH/IC/IM/T/TF/TS/TL —— **八个，不是六个**）系统性缺失且持续至今。
