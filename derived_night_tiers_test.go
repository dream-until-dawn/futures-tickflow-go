...
//
// ⛔ **这一格的身份：它不是守卫，是 derived 判据的【零点】。**
// 它证明「三档 · NoPick · 换月点 · 假阳那一格」在**答案已知的输入**上都被算对了 ——
// 真数据回来的时候，它是唯一能说明「流水线本身没坏」的东西。
// ⇒ 别把它当成一条普通夹具测试删掉，更别在它红的时候去改期望值：
// 期望值来自下面 `ntPlans` 那张构造表，改期望值等于把零点挪到读数上。
...
func buildNightTiers(t *testing.T, root string, syms []tickflow.Symbol) nightTiers {
	...
	for i, bar := range c.Bars {
		...
		// 它满足不了下面那个 `> 0`。实测：把这个子句删掉（补 `_ = other` 让它编译得过），本测试全绿
		// —— 两边各打了一次，读数相同。留着它是为了写清意图 —— 但**别把它读成一道被守着的判据**。
		//
		// ⛔ 它将来会活的那一刻，写成代码就是这一句：把下面分档里的
		//
		//	case a.nightBars > 0 && a.nightVol > 0:
		//
		// 去掉 `&& a.nightVol > 0` 那半句（＝分档改成只看根数、不看量）。
		// 那一刻 `other != sym` 才有射程，**并且要同时补一格会红的输入**：
		// 主连有夜盘根而量 0、别的合约也有夜盘根 —— 那时自己人也会满足 `nightBars > 0`。
