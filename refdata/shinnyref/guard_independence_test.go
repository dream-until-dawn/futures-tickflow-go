...
// ⇒ 所以它护不住本仓的任何一行代码；它护的是一个**关于依赖的假设**：
// 「`json.Decoder` 会把 reader 读到 EOF」。
// 它的失败方向是「Go 换了 `json.Decoder` 的行为」或「`Scan` 从中间提前返回了」——
// 而那两件事发生时，`sawEOF` 会开始在正常路径上误报，**那正是要有人知道的时刻**。
// ⚠️ 写下这一段是因为本仓那条：**一条不会因为我改代码而红的测试，
// 要在它自己身上写明「它不为我的代码把关」** —— 否则下一个人会把它数进覆盖率里。
