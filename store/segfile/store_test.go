...
// ⛔ 而「测到的是一半」这个说法【写轻了】。评审方 2026-09-09 把范围从这一条测试
// 放大到整包，我复现了：
//
//	把 writeMeta 整个换成 os.WriteFile 直接写目标（temp+rename 全删）
//	gofmt 过、go vet 退 0（不是 BUILD）
//	go test ./store/segfile/ ⇒ **ok，整包全绿**
//
// ⇒ **今天没有任何一条测试会因为把 temp+rename 删掉而变红。不是一半，是零。**
// 「一半」听起来还有一半在守着，而实际上一条也没有 —— 这句必须写准。
//
// ⚠️ 而为什么测不出来，推到底之后反而支持「不造假对照组」这个处置：
//
//	直接写      os.WriteFile 会【先截断目标】再写
//	temp+rename 目标在 rename 之前一字节未动
//	⇒ 两者只在【写到一半就失败】时才分得开，而 os.WriteFile 不给注入失败的缝；
//	  要有缝，就得把一个可注入的 writer 写进【生产签名】。
//
