...
// 而下面这一段是**一本账**，不是几条各自带射程的句子：
//
//	`.dat` 这个句柄从 `OpenDat` 交出来、到 `Close` 为止，**只被两条语句写过**：
//	  `dat.go`  的 `f.Truncate(n * RecordSize)`  —— 开机截残尾，它截到的正是整数倍
//	  本文件的 `s.dat.Write(buf)`                —— `buf` 长度 ＝ `len(bars)*RecordSize`
//	**两条都保持整数倍**；而后者的错误**被检查了**，Go 的 `Write` 短写时返回 error
//	⇒ 短写不会被当成成功。
//
// ⛔ **上一版这里写的是「全仓只有一处写入点」，而那是假的**（评审方 2026-09-11 指出）。
// 🔴 而值钱的不是那个数错了，是**两个人各自量、各自量错、错得一模一样**：
// 他没用工具就说「只有一处」，我用的是 `grep "dat\.Write\|dat\.Truncate"` ——
// **两边都按【变量名】划了射程**，而 `OpenDat` 里那一处的接收者叫 `f`，不叫 `dat`。
// ⚠️ 更难看的一格：上一版①自己点名了 `f.Truncate(...)`，②转头就说「只有一处」——
// **同一段话里前后两句互相拆台，而它读起来完全顺**，因为①说「截断」、②说「写入点」，
// 两个词让谁都没把它们算进同一本账。
// ⇒ **把一件事拆成几半来答时，每一半都自带一个默认射程，而那些射程不会自动对齐。**
//
// —— 而正确的问法要**从封闭的那一侧**出发（本仓那条：完备性断言先逐维问封不封闭）——
//
//	按语法找「哪些行长得像写」**不封闭**：往一个 `*os.File` 里写字节有一大把
//	不含 `.Write(` 的写法（`io.Copy` / `fmt.Fprint*` / `bufio` / `NewEncoder`……）。
//	这些写法本包今天一处都没有 —— **而那是「这个包今天没人那么写」，不是判据完备。**
//
//	按句柄找「谁被写」**封闭得多**：句柄的入口只有 `os.OpenFile` / `os.Create`，
//	而本包只开两个文件（`dat.go` 开 `.dat`、`store.go` 开 `.meta` 的临时文件）。
//	⇒ **把每一条写语句的接收者往上追到它的 `Open`**，账就只有两本。
//
// ⛔ **而这一段自己是个陷阱，写在这儿免得下一个人被它绊到**（评审方 2026-09-11 实测）：
// 上一版在这里逐字写了那条 grep **并报了命中数** ——
// 🔴 **而这段注释本身含有那些串，于是它把自己算进了命中里**：
// 照着跑会得到一个比注释里那个数更大的值，然后开始怀疑注释过期了 —— 而它没过期。
// ⇒ 处置两条，都已照做：
//
//	一、**不在注释里写命中数** —— 数字会过期（而且会被这段注释自己顶大），**句子不会**
//	二、真要跑，在末尾加 `| grep -v ':[[:space:]]*//'` 把注释行排掉
//
// 📎 这与本仓那条「**更正说明里【描述】它，别【复制】它**」是同一条，
// 而它这次没被触发的原因也是同一个：**触发按「这一段在讲什么」划（我心里它是
// 「一次判据失误的说明」），危险按「这一段含哪些字符」划** —— 两者不在同一维度。
