...
// 后果写在这儿，因为它是 C3a 的地基：**记录长变了之后，旧 `.dat` 会周期性地
// 被新代码整除、算出一个「没有残尾」的假结论**——而那是个静默的错误答案。
//
// ⚠️ 「周期性」这个词是量出来的，不是估的（2026-09-09；量法：旧长 88，
// 枚举条数 k，看 k*88 % 新长 == 0 的那些）：
//
//	新长 44（减半） 每 1 条一次 ⇒ **100% 的文件都会被误判成干净**
//	新长 176（加倍）每 2 条一次 ⇒ 约 50%
//	新长 80        每 10 条一次 ⇒ 约 10%
//	新长 96        每 12 条一次 ⇒ 约 8%
//
// ⇒ **误判率取决于新旧长度的整除关系，而「减半 / 加倍」正是最可能的改法，
// 也正是最坏的两档。** 这一行原来写的是「一旦变了就会」——太满：
// 在两次误判之间它会正确地报出残尾，**而那恰恰让它更难被发现**。
