...
// ⛔ 2026-09-10 由 1 升到 2：本版把【周期】写进文件名（design.md §十八）。
// 升版本不是为了格式变了（`.meta` 的字段一个没动），是为了让**旧库读不进来** ——
// format=1 的那些库，`Open` 无条件开 `1m.dat`，而当时两个源的 Periods 都只有 Daily
// ⇒ **它们装的是哪个周期没有任何东西记着**。把它们当 1m 收下才是错的。
// ⚠️ 而「旧库全打不开」这件事，在别的设计下会是一项代价；在这一版**它就是正确行为**。
...
//	     它走 D2a（DecideLegacyMeta），因为那个判定要「这个源能不能把那一段【重新给一遍】」，
//	     而那要 Caps.Since，本层没有
//
